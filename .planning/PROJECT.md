...
- No new `thirdparty/` vendoring expected: reuse already-vendored Vulkan-Headers/Vulkan-Loader/MoltenVK (the same stack MNN already builds against); confirm during research whether a SPIR-V compiler toolchain needs adding
- Implement a `RenderProcessor` (parallel to the MNN `ProcessingProcessor` family) built on raw Vulkan that builds the pipeline from schema config and executes a render pass headless (offscreen — no swapchain/window, since this is a distributed compute node), with its own independent `VkInstance`/`VkDevice` coexisting safely alongside MNN's separate internal Vulkan usage, writing output to a texture/buffer consumable the same way inference outputs are today
- End-to-end proof: a real render pass definition executes through the real distributed processing pipeline and produces a verifiable output (e.g. rendered image hash), with zero regressions to the existing MNN inference/retrain suite

**Rendering backend decision (2026-07-29, superseding 2026-07-28's bgfx pivot):** `GeniusVentures/SGProcessingManager#7` explicitly mandates reusing the existing Vulkan stack and prohibits adding another GPU backend or duplicate platform setup — no OpenGL, no SwiftShader, no CPU/software rendering fallback anywhere in scope. The prior bgfx + three-tier-fallback direction (archived, see below) never actually satisfied this constraint — bgfx is itself a new GPU-backend-abstraction engine with its own independent Vulkan init, needing a shared lock with MNN, which is still "duplicate platform setup." This milestone restarts v1.0 as hand-rolled Vulkan only, matching the issue precisely. The original bgfx-based v1.0 workstream is archived intact at `.planning/milestones/ws-sgproc-render-2026-07-29/` — nothing deleted, superseded only.
