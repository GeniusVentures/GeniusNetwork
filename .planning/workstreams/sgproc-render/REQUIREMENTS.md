...
- [x] **XNODE-01b**: `ProcessingValidationCore::ValidateResults` actually diffs two subtasks' per-chunk hashes for the same chunk (fixing the concatenation bug where a genuine mismatch silently passes today)
...
- [x] **SECV-02**: A deliberately wrong/corrupted subtask result is still caught by `ValidateResults` post-fix — proving XNODE-01b/XNODE-02 together aren't loose enough to also mask a genuine defect (counter-test, mirrors v2.1's SECV-01 discipline for this new mechanism)
