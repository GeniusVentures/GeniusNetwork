...
### RENDTOL-02 gap-closure decision (post-verification, 2026-08-20)
- **D-10:** 17-VERIFICATION.md found blending's Round 2 capture fails SC4: `QuantizeByteBuffer`'s floor-to-bucket bit-masking (no rounding tie-break) amplified a real raw 1-unit cross-hardware delta into a 64-unit quantized delta (`maxAbsDelta` 1.0 → 64.0), because the two raw values straddled a `2^N`-aligned bucket boundary. Three options were presented to the user (redesign `QuantizeByteBuffer` to round-to-nearest; fall back to numeric-tolerance comparison for blending; accept as a documented override mirroring VALD-01). **User chose: numeric-tolerance fallback.** Do not redesign `QuantizeByteBuffer` and do not close this as an accepted override — a working fallback path is the required outcome.
- **D-11 (grounding research, verified by direct code read — not yet reflected in any doc):** The numeric-tolerance fallback this decision points to **already exists and is already used in production**, just not in this phase's own verification methodology:
  - `sgns::processing::ProcessingValidationCore::ValidateResults` (`SuperGenius/src/processing/processing_validation_core.cpp`) has an existing `AttemptToleranceFallback` path (Phase 15/XNODE-02) that — on a quantized-hash mismatch — fetches each subtask's **raw** (pre-quantization) output blob via `fetchOutputData`/IPFS and calls `sgprocmanagerdiff::IsByteChunkWithinTolerance`/`IsFloatChunkWithinTolerance` (`SuperGenius/SGProcessingManager/src/util/diff_utils.cpp`) directly on those **raw** bytes.
  - `IsByteChunkWithinTolerance`'s bound for a declared `byteQuantMode=N` is `(1<<N)-1` (63 for N=6), compared against the **raw** delta — NOT the quantized delta. Blending's real raw delta is `1.0` (Round 1, `diff-render-blending.json`) — trivially within a 63-unit bound. **This means production cross-node validation for blending likely already tolerates this divergence correctly today.**
  - The actual gap is narrower than "RENDTOL-02 is broken": **`SuperGenius/SGProcessingManager/tools/capture/capture_diff.cpp`** (the offline two-machine capture-comparison tool this phase's own SC4 methodology uses) never calls `IsByteChunkWithinTolerance`/`IsFloatChunkWithinTolerance` at all — confirmed by direct grep, zero references. It only compares `contentHashMatch` on the two machines' captured (already-quantized, since `byteQuantMode` was live at capture time) artifact bytes, which is a strictly stricter bar than what `ValidateResults` actually enforces in production. Round 1's raw two-machine capture data for blending (`byteQuantMode` absent) already exists on disk (`captures/diff-render-blending.json` and the underlying `.cap` files) — closing this gap likely does not require a new hardware capture round.
  - **Planning implication:** the gap-closure plan should make Phase 17's own SC4 verification methodology apply the same raw-buffer tolerance check `ValidateResults` already applies in production (using Round 1's existing raw capture data for blending), rather than modifying `QuantizeByteBuffer`, `IsByteChunkWithinTolerance`, or any production validation code — none of which the chosen resolution path touches. The exact mechanism (extend `capture_diff.cpp` with a raw-tolerance mode, or a small standalone verification step) is left to planning.
  - Once proven, REQUIREMENTS.md's RENDTOL-02 row and traceability table, and ROADMAP.md's Phase 17 SC4 outcome annotation, need updating to reflect the closed gap (mirroring how RENDTOL-01's annotation was already corrected post-verification).
...
- `SuperGenius/src/processing/processing_validation_core.cpp` (`ValidateResults`/`AttemptToleranceFallback`) — the production numeric-tolerance fallback (Phase 15/XNODE-02) D-11 confirms already runs on raw pre-quantization bytes; the pattern the gap-closure plan should mirror for Phase 17's own SC4 methodology
- `SuperGenius/SGProcessingManager/include/util/diff_utils.hpp`, `src/util/diff_utils.cpp` (`IsByteChunkWithinTolerance`/`IsFloatChunkWithinTolerance`) — the existing tolerance-check functions D-11 grounds; read before deciding how to reuse vs. reimplement for the capture_diff-side check
- `SuperGenius/SGProcessingManager/tools/capture/capture_diff.cpp` — confirmed (grep) to never call the tolerance functions above; only compares `contentHashMatch` on already-quantized bytes — this is the actual gap-closure surface per D-11
