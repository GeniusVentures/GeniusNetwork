...
- **WSL cannot serve as a full third machine for the render fixture** (confirmed 2026-08-11): `RenderProcessor::IsAcceptable()` (`processing_processor_render.cpp:20-24`) and `CapabilityValidator` both hard-require `VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU`/`INTEGRATED_GPU` per Phase 4's locked D-31/D-32 (no software/virtual Vulkan ICD support, by choice). WSL's Vulkan device reports as a type this filter rejects → `RenderProcessor: no acceptable physical device found`. MNN's own Vulkan backend has no such filter, so the MNN-float fixture should still run fine under WSL — only the render fixture is blocked there. Loosening D-31/D-32 to accept virtual/software devices would be a real scope decision, not a quiet patch.
