...
- **User's call:** Add **dedicated** `TransactionManager::DetachChild` + `GeniusNode::DetachChild` (child-initiated) and `TransactionManager::RevokeChild` + `GeniusNode::RevokeChild` (main-initiated) methods — NOT an overload extension of the existing `RegisterChild`/`TransferFunds` methods.
