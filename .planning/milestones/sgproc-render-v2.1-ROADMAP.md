...
### ✅ v2.1 Cross-Hardware Hash Tolerance (Phases 10-13) — SHIPPED 2026-08-13 (closed with an accepted override)
...
- [x] **Phase 13: Re-Validation & Scope Boundary Documentation** - Re-run the ≥2-machine capture/diff cycle with real quantization active to confirm VALD-01, re-confirm SECV-01 at the final precision, and document the milestone's scope boundary (all 3 plans executed 2026-08-12, but goal verification found VALD-01 only partial — render fixture matches cross-hardware, MNN fixture does not; see 13-VERIFICATION.md). Gap-closure Plans 13-04/13-05 widened the quantization grid to S=2^15 and re-measured (2026-08-12): MNN gap narrowed from 12/15 to 1/15 divergent chunk hashes but still not unconditionally clean — see 13-SCOPE-BOUNDARY.md's SC1 Refit section. Plan 13-06 (2026-08-13) characterized the residual gap exactly (one grid-step boundary tie-break) and ruled out FP16 backend opportunism as its cause; accepted via maintainer override in 13-VERIFICATION.md as this milestone's final stopping point (a real fix requires QUANT-CFG-01/XNODE-01b-scale work, scoped to v2.2). (completed 2026-08-13)
...
- [x] 13-06-PLAN.md — Extend capture_diff to numeric-diff per-chunk raw records (not just the trailing combined-hash record), obtain chunk 10's real numeric divergence, and honestly characterize it in 13-SCOPE-BOUNDARY.md — diagnostic only, no further grid-widening proposed
